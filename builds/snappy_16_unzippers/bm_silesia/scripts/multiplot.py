...
output_filename = "HBM_throughput_read_write.pdf"
...
fig.suptitle("Snappy decompression - Silesia Corpus - parallel vhsnunzip")
...
for subfig_n, (result_file, title) in enumerate([(result_file_read, title_read), (result_file_write, title_write)]):
	...
	axs[subfig_n].text(x=10,y=highest_throughput+0.05,s=f"{highest_throughput:.3f} GB/s")

	axs[subfig_n].title.set_text(title)
	if subfig_n == 0:
		axs[subfig_n].set_ylabel("Throughput [GB/s]")
	axs[subfig_n].set_xlabel("Number of active DMAs [N]")
