...
title = f"HBM baseline - read+write - 256 bit AXI-bus - Configuration 3"
...
for i, n_runners in enumerate(set(runners)):
	...
	if n_runners <= 2:
		axs[0].text(x=10,y=highest_throughput-0.4,s=f"{highest_throughput:.3f} GB/s")
	else:
...
for i, n_runners in enumerate(set(runners)):
	...
	if n_runners == 1:
		...
		axs[1].text(x=10,y=highest_throughput-0.55,s=f"{highest_throughput:.3f} GB/s")
	elif n_runners == 2:
		axs[1].axhline(y=highest_throughput, color='r', linestyle='-.')
		axs[1].text(x=10,y=highest_throughput-0.55,s=f"{highest_throughput:.3f} GB/s")
	# else:
